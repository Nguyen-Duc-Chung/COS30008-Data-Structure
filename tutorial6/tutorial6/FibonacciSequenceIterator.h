...
public:
	...
	FibonacciSequenceIterator(const FibonacciSequence& aSequenceObject, uint64_t
		aStart = 1);
	...
	const uint64_t& operator*() const; // return current Fibonacci number
		...
		FibonacciSequenceIterator operator++(int); // postfix (extra unusedargument)
